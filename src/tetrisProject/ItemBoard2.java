...
public class ItemBoard2 extends JComponent {
 ...
 // 아이템 타입별 이미지를 저장하는 해시맵
    ...
    public ItemBoard2(Game game, int width, int height, int maxItems) {
...
//        addItem(1);
    	...
    	synchronized(itemQueue) {
	        ...
	        if (itemImage != null) {
	            if (itemQueue.size() >= maxItems) {
	                itemQueue.poll(); // 맨 위의 아이템을 제거합니다.
	            }
	            itemQueue.offer(itemImage); // 새로운 아이템을 추가합니다.
    ...
    private class ItemBoardComponent extends JComponent {
        ...
        public Dimension getPreferredSize() {
            return new Dimension(width * 9, height * 9); // 단위 유닛 블록 크기 설정
        }
        ...
        private void doPaintComponent(Graphics g) {
        	synchronized(itemQueue) {
	        	super.paintComponent(g);
	        	g.setColor(new Color(33, 95, 177)); //디폴트 : 16, 25, 41
	            ...
	            for (BufferedImage item : itemQueue) {
	                g.drawImage(item, x, 0, itemWidth, itemHeight, this);
	                System.out.println("wrote");
	                x += itemWidth;
	            }
            }
